...
| Command | Purpose |
|---------|---------|
| `--list <keyring.asc>...` | List master keys + subkeys with `[S]`/`[E]` capability |
| `--generate ...` | Generate a key pair (armored `<name>-public.asc` / `<name>-secret.asc`) |
| `--encrypt [file]` | Encrypt to KEY/PASS layers, sign, optional compound attachments |
| `--decrypt [file]` | Decrypt nested messages, save attachments, verify signatures |

Each command supports `--help`. Key selection is **explicit**: `--layer KEY:file[#id]:ALGO` /
...
## Custom AEAD tags (100-102 Serpent-EAX, 103 ChaCha20-Poly1305)

All four custom symmetric tags are AEAD streams (no MDC; `getIntegrityCalculator()` returns `null`), emitted as
LibrePGP-style **AEAD Encrypted Data packets (tag 20, version 1)** that BC 1.84 parses back. Tag **104 is
reserved/unused**. Shared framing lives in `CustomAeadEncryptor`, which dispatches on the tag to an internal
`AeadEngine` (interface: `init(forEnc, nonce)`, `updateAad(bytes)`, `finish(in, off, len)`):
...
- **100-102 | Serpent-128/192/256** (`SerpentTags`): lightweight `EAXBlockCipher(new SerpentEngine())` +
  `AEADParameters(key, 128, nonce)`. Packet byte `aeadAlgorithm=EAX(1)` → BC derives a **16-byte** IV; gpg shows
  `cipher=100/101/102 aead=1 cb=10`. User-facing labels remain "Serpent-128/192/256".
...
(103 → GCM/12B; Serpent → EAX/16B). Editing a tag's mode means keeping this in sync.
...
- Chunk nonce = IV xor chunk index (last 8 bytes of the IV; works for 12/16-byte IVs).
...
- EAX decrypt wraps `InvalidCipherTextException` into `IOException`. **Note**: BC 1.84's `EAXBlockCipher` rejects
  nonce reuse for encryption ("cannot reuse nonce for EAX encryption") — the monotonic chunk index (final tag on the
  *next* index) keeps nonces distinct.
...
- Decrypt factories `CustomAwarePublicKeyDataDecryptorFactory` / `CustomAwarePBEDataDecryptorFactory` override
  ...
  (100-103) they build `CustomAeadEncryptor(sessionKey.getAlgorithm(), sessionKey.getKey(), iv, chunkSize)`, else
